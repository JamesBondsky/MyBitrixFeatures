...
define("FOLDER_PATH_BOT", $_SERVER["DOCUMENT_ROOT"] . "/chat_bot");

$appsConfig     = array();
$configFileName = '/config_' . trim(str_replace('.', '_', $_REQUEST['auth']['domain'])) . '.php';
if (file_exists(FOLDER_PATH_BOT . $configFileName)) {
    include_once FOLDER_PATH_BOT . $configFileName;
}
    ...
    if ($_REQUEST['event'] == 'ONIMBOTJOINCHAT') {
        ...
        if ($_REQUEST['event'] == 'ONIMBOTDELETE') {
            ...
            if ($_REQUEST['event'] == 'ONAPPINSTALL') {
                // handler for events
                $handlerBackUrl = 'https://' . $_SERVER['SERVER_NAME'] . (in_array($_SERVER['SERVER_PORT'],
...
function saveParams($params) {
    ...
    $config .= "?>";
    $configFileName = '/config_' . trim(str_replace('.', '_', $_REQUEST['auth']['domain'])) . '.php';
    file_put_contents(FOLDER_PATH_BOT . $configFileName, $config);
    return true;
}
/**
...
function writeToLog($data, $title = '') {
    ...
    $log .= print_r($data, 1);
    $log .= "\n------------------------\n";
    file_put_contents(FOLDER_PATH_BOT . '/imbot.log', $log, FILE_APPEND);
    return true;
}
/**
